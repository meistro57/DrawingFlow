        ...
        <div class="py-8">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
                        ...
                        >
                            <div :class="[card.tone, card.text, 'bg-gradient-to-br p-5']">
                                <p class="text-xs font-semibold uppercase tracking-[0.2em] opacity-80">{{ card.name }}</p>
                ...
                <div class="mb-8 grid grid-cols-1 gap-8 xl:grid-cols-3">
                    <section class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
                        <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4 dark:border-slate-800">
                            <div>
                                <h2 class="text-lg font-medium text-gray-900 dark:text-slate-100">Assigned Requests</h2>
                                <p class="mt-1 text-xs text-gray-500 dark:text-slate-400">Prioritized by urgency and due date</p>
                            </div>
                            <Link :href="route('drawing-requests.index')" class="text-sm font-medium text-primary-600 hover:text-primary-800 dark:text-primary-400 dark:hover:text-primary-300">View All</Link>
                        </div>
                        <div class="flex flex-wrap gap-2 border-b border-gray-200 px-6 py-3 dark:border-slate-800">
                            <Link
                                v-for="filter in queueFilters"
                                :key="filter.value"
                                :href="route('dashboard', filter.value === 'all' ? {} : { queue_filter: filter.value })"
                                preserve-scroll
                            ...
                            >
                                ...
                                <span class="rounded-full bg-black/6 px-2 py-0.5 text-[10px] dark:bg-white/10">{{ filter.count }}</span>
                            </Link>
                        </div>
                        <div class="divide-y divide-gray-200 dark:divide-slate-800">
                            ...
                            <div v-if="!my_queue?.length" class="px-6 py-8 text-center text-sm text-gray-500 dark:text-slate-400">
                                <span v-if="my_queue_filter === 'all'">No requests are assigned to you right now.</span>
                    ...
                    <section class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
                        <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4 dark:border-slate-800">
                            <div>
                                <h2 class="text-lg font-medium text-gray-900 dark:text-slate-100">Submittals Needing Action</h2>
                                <p class="mt-1 text-xs text-gray-500 dark:text-slate-400">Drafts, revisions, and follow-up items</p>
                            </div>
                            <Link :href="route('submittals.index')" class="text-sm font-medium text-primary-600 hover:text-primary-800 dark:text-primary-400 dark:hover:text-primary-300">View All</Link>
                        </div>
                        <div class="divide-y divide-gray-200 dark:divide-slate-800">
                            <Link
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="min-w-0 flex-1">
                                        <p class="truncate text-sm font-medium text-gray-900 dark:text-slate-100">
                                            ...
                                            <span class="font-normal text-gray-500 dark:text-slate-400">Rev {{ submittal.revision }}</span>
                                        </p>
                                        <p class="mt-1 text-xs text-gray-500 dark:text-slate-400">
                                            <span v-if="submittal.project">{{ submittal.project.name }}</span>
                                            <span v-if="submittal.drawing_request"> · {{ submittal.drawing_request.title }}</span>
                                        </p>
                                    </div>
                                    <StatusBadge :status="submittal.status" class="ml-3" />
                                </div>
                            </Link>
                            <div v-if="!my_submittal_work?.length" class="px-6 py-8 text-center text-sm text-gray-500 dark:text-slate-400">
                    ...
                    <section class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
                        <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4 dark:border-slate-800">
                            <div>
                                <h2 class="text-lg font-medium text-gray-900 dark:text-slate-100">Fab Assignments</h2>
                                <p class="mt-1 text-xs text-gray-500 dark:text-slate-400">What is currently on your fabrication plate</p>
                            </div>
                            <Link :href="route('fab-queue.index')" class="text-sm font-medium text-primary-600 hover:text-primary-800 dark:text-primary-400 dark:hover:text-primary-300">View All</Link>
                        </div>
                        <div class="divide-y divide-gray-200 dark:divide-slate-800">
                            <Link
                            ...
                            >
                                <div class="flex items-center justify-between gap-3">
                                    <div class="min-w-0 flex-1">
                                        <p class="truncate text-sm font-medium text-gray-900 dark:text-slate-100">
                                            {{ entry.queue_number }}
                                            <span v-if="entry.project" class="font-normal text-gray-500 dark:text-slate-400">· {{ entry.project.name }}</span>
                                        </p>
                                        <p class="mt-1 text-xs text-gray-500 dark:text-slate-400">
                                            <span v-if="entry.submittal">{{ entry.submittal.submittal_number }} Rev {{ entry.submittal.revision }}</span>
                                        </p>
                                    </div>
                                    <div class="text-right">
                                        <StatusBadge :status="entry.status" />
                                        <p class="mt-2 text-[11px] font-semibold uppercase tracking-wide text-gray-500 dark:text-slate-400">
                                            Priority {{ entry.priority }}
                                        </p>
                                    </div>
                                </div>
                            </Link>
                            <div v-if="!my_fab_assignments?.length" class="px-6 py-8 text-center text-sm text-gray-500 dark:text-slate-400">
                ...
                <div class="grid grid-cols-1 gap-8 lg:grid-cols-2">
                    <section class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
                        <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4 dark:border-slate-800">
                            <h2 class="text-lg font-medium text-gray-900 dark:text-slate-100">Recent Drawing Requests</h2>
                            <Link :href="route('drawing-requests.index')" class="text-sm font-medium text-primary-600 hover:text-primary-800 dark:text-primary-400 dark:hover:text-primary-300">View All</Link>
                        </div>
                        <div class="divide-y divide-gray-200 dark:divide-slate-800">
                            <Link
                    ...
                    <section class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
                        <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4 dark:border-slate-800">
                            <h2 class="text-lg font-medium text-gray-900 dark:text-slate-100">Recent Submittals</h2>
                            <Link :href="route('submittals.index')" class="text-sm font-medium text-primary-600 hover:text-primary-800 dark:text-primary-400 dark:hover:text-primary-300">View All</Link>
                        </div>
                        <div class="divide-y divide-gray-200 dark:divide-slate-800">
                            <Link
