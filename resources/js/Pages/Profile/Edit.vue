    ...
    <AppLayout>
        ...
        <div class="py-8">
            <div class="mx-auto grid max-w-5xl gap-6 px-4 sm:px-6 lg:grid-cols-[1.1fr_0.9fr] lg:px-8">
                <section class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    <div class="flex flex-col gap-6 sm:flex-row sm:items-center">
                        <div class="relative flex h-28 w-28 items-center justify-center overflow-hidden rounded-3xl bg-slate-200 text-3xl font-semibold text-slate-700 dark:bg-slate-800 dark:text-slate-200">
                            <img
                                v-if="avatarPreview"
                                :src="avatarPreview"
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="mt-2 inline-flex rounded-full bg-primary-50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-primary-700 dark:bg-primary-950/50 dark:text-primary-300">
                                {{ user?.role }}
                            </p>
                        </div>
                    </div>
                    ...
                    <form class="mt-8 space-y-4" @submit.prevent="submitAvatar">
                        <div>
                            <label for="avatar" class="block text-sm font-medium text-gray-700 dark:text-slate-200">
                                Avatar
                            </label>
                            <input
                                id="avatar"
                            ...
                            <p v-if="avatarForm.errors.avatar" class="mt-2 text-sm text-red-600 dark:text-red-400">
                ...
                <section class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    ...
                    <form class="mt-6 space-y-4" @submit.prevent="submitPassword">
                        <div>
                            <label for="current_password" class="block text-sm font-medium text-gray-700 dark:text-slate-200">
                                Current password
                            </label>
                            <input
                                id="current_password"
                            ...
                            />
                            <p v-if="passwordForm.errors.current_password" class="mt-2 text-sm text-red-600 dark:text-red-400">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                class="mt-2 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-primary-500 focus:ring-primary-500 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100"
                            />
                            <p v-if="passwordForm.errors.password" class="mt-2 text-sm text-red-600 dark:text-red-400">
                                {{ passwordForm.errors.password }}
                            </p>
                        </div>

                        <div>
                            <label for="password_confirmation" class="block text-sm font-medium text-gray-700 dark:text-slate-200">
