...
function assignUser() {
    router.post(route('drawing-requests.assign', props.drawingRequest.id), {
    ...
    <AppLayout>
        ...
        <div class="py-8">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="mb-6">
                    <Link :href="route('drawing-requests.index')" class="text-sm text-gray-500 hover:text-gray-700">&larr; Back to Drawing Requests</Link>
                </div>
                ...
                <div class="flex items-start justify-between mb-8">
                    <div>
                        <div class="flex items-center space-x-3">
                            ...
                            <span :class="[priorityClasses[drawingRequest.priority] || 'bg-gray-100 text-gray-800', 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium']">
                                {{ drawingRequest.priority ? drawingRequest.priority.charAt(0).toUpperCase() + drawingRequest.priority.slice(1) : 'Normal' }}
                            </span>
                        </div>
                        <p class="mt-1 text-lg text-gray-700">{{ drawingRequest.title }}</p>
                        <p class="mt-1 text-sm text-gray-500">
                            <Link v-if="drawingRequest.project" :href="route('projects.show', drawingRequest.project.id)" class="text-primary-600 hover:text-primary-800">
                                {{ drawingRequest.project.name }}
                            </Link>
                            <span v-if="drawingRequest.customer"> &middot;
                                <Link :href="route('customers.show', drawingRequest.customer.id)" class="text-primary-600 hover:text-primary-800">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="lg:col-span-2 space-y-8">
                        <div class="bg-white shadow-sm rounded-lg border border-gray-200">
                            ...
                            <dl class="divide-y divide-gray-200">
                                <div v-if="drawingRequest.description" class="px-6 py-4 grid grid-cols-3 gap-4">
                                    ...
                                    <dd class="text-sm text-gray-900 col-span-2 whitespace-pre-line">{{ drawingRequest.description }}</dd>
                                </div>
                                <div class="px-6 py-4 grid grid-cols-3 gap-4">
                                    <dt class="text-sm font-medium text-gray-500">Drawing Type</dt>
                                    <dd class="text-sm text-gray-900 col-span-2">{{ drawingRequest.drawing_type || '-' }}</dd>
                                </div>
                                <div class="px-6 py-4 grid grid-cols-3 gap-4">
                                    <dt class="text-sm font-medium text-gray-500">Job Number</dt>
                                    <dd class="text-sm text-gray-900 col-span-2">{{ drawingRequest.job_number || '-' }}</dd>
                                </div>
                                <div class="px-6 py-4 grid grid-cols-3 gap-4">
                                    <dt class="text-sm font-medium text-gray-500">Customer Address</dt>
                                    <dd class="text-sm text-gray-900 col-span-2">{{ drawingRequest.customer_address || '-' }}</dd>
                                </div>
                                <div class="px-6 py-4 grid grid-cols-3 gap-4">
                                    <dt class="text-sm font-medium text-gray-500">Required Date</dt>
                                    <dd class="text-sm text-gray-900 col-span-2">{{ drawingRequest.required_date || '-' }}</dd>
                                </div>
                                <div class="px-6 py-4 grid grid-cols-3 gap-4">
                                    <dt class="text-sm font-medium text-gray-500">Requested Date</dt>
                                    <dd class="text-sm text-gray-900 col-span-2">{{ drawingRequest.requested_date || '-' }}</dd>
                                </div>
                                <div v-if="drawingRequest.notes" class="px-6 py-4 grid grid-cols-3 gap-4">
                                    <dt class="text-sm font-medium text-gray-500">Notes</dt>
                                    <dd class="text-sm text-gray-900 col-span-2 whitespace-pre-line">{{ drawingRequest.notes }}</dd>
                                </div>
                            </dl>
                        </div>
                        ...
                        <div class="bg-white shadow-sm rounded-lg border border-gray-200 overflow-hidden">
                            ...
                            <table v-if="drawingRequest.submittals?.length" class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="divide-y divide-gray-200">
                                    <tr v-for="submittal in drawingRequest.submittals" :key="submittal.id" class="hover:bg-gray-50">
                                        <td class="px-6 py-4 text-sm">
                                            <Link :href="route('submittals.show', submittal.id)" class="text-primary-600 hover:text-primary-800 font-medium">
                                                {{ submittal.submittal_number }}
                                            </Link>
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-900">Rev {{ submittal.revision }}</td>
                                        <td class="px-6 py-4 text-sm text-gray-500">{{ submittal.submitted_by?.name || '-' }}</td>
                                        <td class="px-6 py-4"><StatusBadge :status="submittal.status" /></td>
                                    </tr>
                                </tbody>
                            </table>
                        ...
                        <div class="bg-white shadow-sm rounded-lg border border-gray-200 p-6">
                            ...
                            <dl class="space-y-4">
                                <div>
                                    ...
                                    <dd class="text-sm font-medium text-gray-900">{{ drawingRequest.requested_by?.name || '-' }}</dd>
                                </div>
                                <div>
                                    <dt class="text-xs text-gray-500">Assigned To</dt>
                                    <dd class="text-sm font-medium text-gray-900">{{ drawingRequest.assigned_to?.name || 'Unassigned' }}</dd>
                                </div>
                            </dl>
                        </div>
                    </div>
