...
function assignUser() {
    router.post(route('fab-queue.assign', props.entry.id), {
    ...
    <AppLayout>
        ...
        <div class="py-8">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="mb-6">
                    <Link :href="route('fab-queue.index')" class="text-sm text-gray-500 hover:text-gray-700">&larr; Back to Fab Queue</Link>
                </div>
                ...
                <div class="flex items-start justify-between mb-8">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500">
                            ...
                            <span v-if="entry.submittal?.customer"> &middot; {{ entry.submittal.customer.name }}</span>
                        </p>
                    </div>
                            ...
                            <dl class="divide-y divide-gray-200">
                                <div class="px-6 py-4 grid grid-cols-3 gap-4">
                                    ...
                                    <dd class="text-sm text-gray-900 col-span-2">
                                        <Link v-if="entry.submittal" :href="route('submittals.show', entry.submittal.id)" class="text-primary-600 hover:text-primary-800">
                                ...
                                <div class="px-6 py-4 grid grid-cols-3 gap-4">
                                    ...
                                    <dd class="text-sm text-gray-900 col-span-2">
                                        <Link v-if="entry.submittal?.drawing_request" :href="route('drawing-requests.show', entry.submittal.drawing_request.id)" class="text-primary-600 hover:text-primary-800">
                                            {{ entry.submittal.drawing_request.request_number }} - {{ entry.submittal.drawing_request.title }}
                                        </Link>
                                    </dd>
                                </div>
                                <div class="px-6 py-4 grid grid-cols-3 gap-4">
                                    <dt class="text-sm font-medium text-gray-500">Assigned To</dt>
                                    <dd class="text-sm text-gray-900 col-span-2">{{ entry.assigned_to?.name || 'Unassigned' }}</dd>
                                </div>
                                <div class="px-6 py-4 grid grid-cols-3 gap-4">
                                ...
                                <div class="px-6 py-4 grid grid-cols-3 gap-4">
                                    ...
                                    <dd class="text-sm text-gray-900 col-span-2 whitespace-pre-line">{{ entry.material_requirements || '-' }}</dd>
                                </div>
                                <div class="px-6 py-4 grid grid-cols-3 gap-4">
                                    <dt class="text-sm font-medium text-gray-500">CNC Files Attached</dt>
                                    <dd class="text-sm text-gray-900 col-span-2">{{ entry.cnc_files_attached ? 'Yes' : 'No' }}</dd>
                                </div>
                            </dl>
                        </div>
                        ...
                        <div class="bg-white shadow-sm rounded-lg border border-gray-200">
                            ...
                            <div class="p-6 space-y-4">
                                <div>
                                    ...
                                    <textarea v-model="shopNotes" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm" placeholder="Notes for the shop floor..."></textarea>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">General Notes</label>
                                    <textarea v-model="notes" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm"></textarea>
                                </div>
                                <div class="flex justify-end">
                                    <button @click="saveNotes" class="inline-flex items-center px-4 py-2 bg-primary-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-primary-700 transition">
                                        Save Notes
                                    </button>
                                </div>
                            </div>
