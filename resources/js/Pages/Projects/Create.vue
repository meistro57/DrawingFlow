...
const form = useForm({
    project_number: '',
    name: '',
    customer_id: props.preselected_customer_id || '',
    description: '',
    address: '',
    city: '',
    state: '',
    ...
    <AppLayout>
        ...
        <div class="py-8">
            <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="mb-6">
                    <Link :href="route('projects.index')" class="text-sm text-gray-500 hover:text-gray-700">&larr; Back to Projects</Link>
                    <h1 class="mt-2 text-2xl font-bold text-gray-900">Add Project</h1>
                </div>
                ...
                <form @submit.prevent="submit" class="bg-white shadow-sm rounded-lg border border-gray-200">
                    <div class="p-6 space-y-6">
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            <div>
                                ...
                                <input v-model="form.project_number" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm" placeholder="e.g. P-2026-001" />
                        ...
                        <div>
                            ...
                            <p v-if="form.errors['attachments.0']" class="mt-1 text-sm text-red-600">{{ form.errors['attachments.0'] }}</p>

                            <p v-if="form.attachments.length" class="mt-2 text-xs font-medium text-gray-600">
                                {{ form.attachments.length }} file(s) selected
                            </p>
                            <ul v-if="form.attachments.length" class="mt-2 divide-y divide-gray-200 rounded-md border border-gray-200">
                                <li v-for="(file, index) in form.attachments" :key="`${file.name}-${index}`" class="flex items-center justify-between px-3 py-2">
                                    <div class="min-w-0">
                                        <p class="truncate text-sm font-medium text-gray-900">{{ file.name }}</p>
                                        <p class="text-xs text-gray-500">{{ formatFileSize(file.size) }}</p>
                                    </div>
                                    <button type="button" class="text-xs font-semibold text-red-600 hover:text-red-700" @click="removeAttachment(index)">
                                        Remove
                                    </button>
                                </li>
                            </ul>
                    ...
                    <div class="border-t border-gray-200 px-6 py-4 bg-gray-50 flex justify-end space-x-3">
                        <Link :href="route('projects.index')" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest bg-white hover:bg-gray-50 transition">
                            Cancel
                        </Link>
