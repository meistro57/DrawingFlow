...
export function useSubmittalStatus(submittal) {
  ...
  const canSubmit = computed(() => {
    return submittal.status === 'ready_to_submit' && submittal.permissions?.can_submit;
  });

  const canEdit = computed(() => {
    return (
      ['draft', 'ready_to_submit'].includes(submittal.status) && submittal.permissions?.can_edit
    );
  });
